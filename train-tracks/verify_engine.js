#!/usr/bin/env node
/* GENERATED in-engine verifier for train-tracks — pattern follows akari/verify_engine.js.
 ...
 * Levels + solution_grid generated deterministically by generateLevels(); per level: startLevel(i); playerGrid = solution_grid; checkWin() must set game.solved.
 * Usage: node train-tracks/verify_engine.js   (cwd = repo root)
 */
const fs = require('fs');
const vm = require('vm');
...
const DRIVER = "(function(){\nlet pass=0,fail=0,fails=[],failIdx=[],structOK=0;\ngame.levels=generateLevels();\nfor(let i=0;i<game.levels.length;i++){const L=game.levels[i];\n try{\n  L.solution_grid=L.grid;\n  startLevel(i);\n  game.playerGrid=L.grid.map(function(r){return r.slice();});\n  game.solved=false;\n  let ex2=null;\n  try{checkWin();}catch(e2){ex2=String(e2.message).slice(0,40);}\n  const engineWin=game.solved===true;\n  const lvl=game.level;\n  let cluesOK=true;\n  for(let r=0;r<lvl.rows;r++){let cnt=0;for(let c=0;c<lvl.cols;c++)if(game.playerGrid[r][c]>0)cnt++;if(cnt!==lvl.rowClues[r])cluesOK=false;}\n  for(let c=0;c<lvl.cols;c++){let cnt=0;for(let r=0;r<lvl.rows;r++)if(game.playerGrid[r][c]>0)cnt++;if(cnt!==lvl.colClues[c])cluesOK=false;}\n  let traceOK=false;\n  try{\n   let dir=OPP[lvl.start.dir];\n   let cr=lvl.start.r,cc=lvl.start.c;\n   const visited={};visited[cr+','+cc]=true;\n   let plen=1;\n   let iter=lvl.rows*lvl.cols+5,done=false;\n   while(iter-->0){\n    const nr=cr+DY[dir],nc=cc+DX[dir];\n    if(nr<0||nr>=lvl.rows||nc<0||nc>=lvl.cols){done=(cr===lvl.end.r&&cc===lvl.end.c&&dir===lvl.end.dir);break;}\n    const key=nr+','+nc;\n    if(visited[key])break;\n    const np=game.playerGrid[nr][nc];\n    if(np===0)break;\n    const nconn=PIECES[np].conn;\n    if(!(nconn&OPP[dir]))break;\n    const nd=nconn&~OPP[dir];\n    if(nd===0)break;\n    visited[key]=true;plen++;cr=nr;cc=nc;dir=nd;\n    if(cr===lvl.end.r&&cc===lvl.end.c){done=(dir===lvl.end.dir);break;}\n   }\n   traceOK=done&&plen===lvl.rows*lvl.cols&&Object.keys(visited).length===lvl.rows*lvl.cols;\n  }catch(e3){traceOK=false;}\n  if(engineWin)pass++;else{fail++;failIdx.push(i+1);fails.push('L'+(i+1)+' engineNoWin'+(cluesOK?'':' clues')+(traceOK?'':' trace')+(ex2?' EX:'+ex2:''));}\n  if(cluesOK&&traceOK)structOK++;\n }catch(e){fail++;failIdx.push(i+1);fails.push('L'+(i+1)+' EX:'+String(e.message).slice(0,60));}\n}\nreturn {pass:pass,fail:fail,total:game.levels.length,failIdx:failIdx,fails:fails.slice(0,15),verdict:fail===0?'PASS':'FAIL',extra:{structurallyValidSolutions:structOK,note:'engine traceTrack() bug: initial dir uses outward entry stub lvl.start.dir instead of OPP[lvl.start.dir], so in-engine checkWin never fires; extra.structurallyValidSolutions = levels whose solution_grid passes row/col clues + corrected full trace'}};\n})()";
...
console.log('train-tracks in-engine verification: ' + out.pass + '/' + out.total + ' levels, verdict=' + out.verdict);
