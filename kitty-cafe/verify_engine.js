#!/usr/bin/env node
/* kitty-cafe — permanent alias page redirecting to kitchen-rush (no engine by design).
...
T('refresh-target', last(refresh) === 'kitchen-rush', refresh);
T('js-target', last(js) === 'kitchen-rush', js);
T('canonical-target', last(canon) === 'kitchen-rush', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'kitchen-rush', 'index.html');
...
T('target-is-real-game', tok, 'kitchen-rush/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'kitchen-rush' } };
