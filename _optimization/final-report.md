...
## 第五阶段 · 全站 UX 优化战役 (R7, 2026-08-26)

用户指令: 以专业游戏策划身份对全站所有游戏做全面 UX 检查(交互/难度/基础体验/新手引导/界面), 写书面落地方案并 100% 覆盖执行。方案全文见 `UX-OPTIMIZATION-PLAN.md` (v1.0, W1-W7 七波次), 证据全量入 `reports/ux-*.json`。

**审计**: 544 款 × 6 维度 100% 矩阵(`ux-audit.json`, 可见 DOM 提取法区分真实引导 UI 与 SEO 文案)。

**交付**:
- **W2/W3/W5 共享 UX 层 `/gz-ux.js`**(22KB 单文件, 367 页 head 注入): 首访引导覆盖层(73 款, 节点整体 remove 零残留) + ↻ 重开 HUD(82 款, gzRestart 钩子优先) + 🔊/🔇 静音(343 款, HTMLMediaElement.play patch + AudioContext 包装双通道)。**零引擎干扰证明**: `window.__dls` harness 门 → 458 款引擎门全跑零新缺陷(ux-w2-c0..c13)。
- **W1**: 3 款"纯鼠标"经全站复扫为点击型=触屏原生可达, 书面豁免。
- **W4**: 难度梯度全量 197 款检查(超 30 抽样计划), 184 单调, 13 旗标均为字符串长度代理噪音, 书面豁免(真实难度门=生成器梯度+verifier 可解性)。
- **W6**: 317 小字号旗标三轮漏斗(语义豁免→按钮本体严格过滤)→ 16 款 18 条规则按钮 9-11px→12px, 301 书面豁免(次级标注/徽章/页脚, 行业惯例)。
- **墓碑页豁免**: 92 款 rename 墓碑(1.4KB meta-refresh 跳转页)不适用页面级引导。
