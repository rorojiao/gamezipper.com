#!/usr/bin/env node
/* hoop-stack — permanent alias page redirecting to magic-sort (no engine by design).
...
T('refresh-target', last(refresh) === 'magic-sort', refresh);
T('js-target', last(js) === 'magic-sort', js);
T('canonical-target', last(canon) === 'magic-sort', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'magic-sort', 'index.html');
...
T('target-is-real-game', tok, 'magic-sort/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'magic-sort' } };
