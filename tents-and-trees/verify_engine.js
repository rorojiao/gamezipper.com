#!/usr/bin/env node
/* tents-and-trees — permanent alias page redirecting to tents (no engine by design).
...
T('refresh-target', last(refresh) === 'tents', refresh);
T('js-target', last(js) === 'tents', js);
T('canonical-target', last(canon) === 'tents', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'tents', 'index.html');
...
T('target-is-real-game', tok, 'tents/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'tents' } };
