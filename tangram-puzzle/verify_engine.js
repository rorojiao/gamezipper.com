#!/usr/bin/env node
/* tangram-puzzle — permanent alias page redirecting to tangram (no engine by design).
...
T('refresh-target', last(refresh) === 'tangram', refresh);
T('js-target', last(js) === 'tangram', js);
T('canonical-target', last(canon) === 'tangram', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'tangram', 'index.html');
...
T('target-is-real-game', tok, 'tangram/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'tangram' } };
