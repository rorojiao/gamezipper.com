#!/usr/bin/env node
/* words-klondike — permanent alias page redirecting to word-card-sort (no engine by design).
...
T('refresh-target', last(refresh) === 'word-card-sort', refresh);
T('js-target', last(js) === 'word-card-sort', js);
T('canonical-target', last(canon) === 'word-card-sort', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'word-card-sort', 'index.html');
...
T('target-is-real-game', tok, 'word-card-sort/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'word-card-sort' } };
