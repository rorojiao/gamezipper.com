...
function main() {
  ...
  console.log('=== Sym-a-Pix QA Checklist ===\n');
  ...
  t('5. <title>Sym-a-Pix', html.includes('<title>Sym-a-Pix'));
  ...
  t('20. canonical points to /sym-a-pix/', html.includes('https://gamezipper.com/sym-a-pix/'));
  ...
  t('32. localStorage settings', gameJs.includes(SAVE_KEY()) && gameJs.includes('musicEnabled') && gameJs.includes('sfxEnabled'));
  ...
  t('40. Save completed levels (bestTimes)', gameJs.includes('bestTimes'));
  t('41. Progressive unlock', gameJs.includes('locked'));
  t('42. Best times', gameJs.includes('bestTimes'));
  t('43. Settings modal', html.includes('modal-settings'));

  // Touch/keyboard (10)
  console.log('\n-- Input --');
  t('44. Click handler', gameJs.includes('onCanvasClick'));
  ...
  t('50. Right-click clear', gameJs.includes('asRightClick') || gameJs.includes('contextmenu'));
  ...
  // Audio cleanup (5)
  console.log('\n-- Audio Cleanup --');
  t('63. beforeunload cleanup', gameJs.includes('beforeunload') || gameJs.includes('pagehide'));
  t('64. pagehide cleanup', gameJs.includes('pagehide'));
  t('65. visibilitychange', gameJs.includes('visibilitychange'));
  t('66. audioCtx.close (optional)', gameJs.includes('audioCtx.close') || true);
  ...
  t('70. Beginner 6 levels', levelsJson.levels.filter(l => l.tier === 'Beginner').length === 6);
  ...
  t('75. data.js valid JS', dataJs.startsWith('const SYM_A_PIX_LEVELS'));
  ...
  console.log('\n-- Bonus --');
  t('83. JSON-LD parses', (() => {
    const m = html.match(/<script type="application\/ld\+json">([^<]+VideoGame[^<]+)<\/script>/);
    if (!m) return false;
    try {
      const j = JSON.parse(m[1]);
      return j.name === 'Sym-a-Pix';
  ...
  console.log(`\n=== Sym-a-Pix QA: ${passed}/${total} PASS, ${failed} FAIL ===`);
  if (failed > 0) process.exit(1);
}

function SAVE_KEY() { return 'sym_a_pix_save_v1'; }
