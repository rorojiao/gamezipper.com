#!/usr/bin/env node
/**
 * Antelope Path — in-engine verification via JSDOM.
 * Loads data.js + game.js, then walks every level to confirm the engine
 * solves it correctly using the embedded ANTELOPE_PATH_DATA.
 */
const fs = require('fs');
const path = require('path');
const { JSDOM } = require('jsdom');

const DIRS = [[3,1],[1,3],[3,-1],[1,-3],[-3,1],[-1,3],[-3,-1],[-1,-3]];
...
<div id="levels-list"></div>
  ...
  // Run data.js (defines window.ANTELOPE_PATH_DATA)
  window.eval(dataJs);
  if (!window.ANTELOPE_PATH_DATA) {
    console.error('FAIL: ANTELOPE_PATH_DATA not loaded');
    process.exit(1);
  }
  ...
  const data = window.ANTELOPE_PATH_DATA;
  ...
  for (const lvl of levels) {
    ...
    // Check all hops in solution_path are antelope (3,1) leaper
    ...
    for (let i = 0; i < path.length - 1; i++) {
      ...
      if (!(hops.size === 2 && hops.has(1) && hops.has(3))) { badHop = true; break; }
    }
