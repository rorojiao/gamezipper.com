#!/usr/bin/env node
/**
 * Antelope Path — Independent Node.js verification.
 * Re-implements the antelope (3,1) leaper BFS from scratch (no shared code with gen_levels.py).
 * Validates each level has a unique shortest antelope path.
 */
...
function main() {
  ...
  for (const lvl of levels) {
    ...
    // Check all hops are valid antelope (3,1) leaper
    let badHop = false;
    const seen = new Set();
    for (const p of claimedPath) {
      const k = p.join(',');
    ...
    for (let i = 0; i < claimedPath.length - 1; i++) {
      ...
      if (!(hops.size === 2 && hops.has(1) && hops.has(3))) {
        badHop = true; break;
      }
    }
