// Bishop Path game engine
//
// Rules:
// - N x N grid with walls (impassable cells).
// - Player draws a path from START to GOAL using only BISHOP moves:
//   any number of cells diagonally in one of 4 directions (NW, NE, SW, SE).
// - Path length = number of moves (edges). Win when path reaches GOAL.
// - Each level has a unique shortest bishop path.
//
// Controls:
// - Click/tap START, then click any bishop-reachable cell to extend.
// - Backspace / Undo button: remove last step.
// - Enter / Check button: verify path reaches goal.
// - H: hint (3 per level) — extends path along the optimal solution.
// - R: restart level.

(function() {
  'use strict';

  const SAVE_KEY = 'bishoppath_save_v1';
  const HINT_LIMIT = 3;

  // 4 bishop diagonals: any number of cells
  const BISHOP_DIRS = [
    [-1, -1], [-1, 1], [1, -1], [1, 1]
  ];
  ...
  function bindEvents() {
    ...
    dom.btnNext.addEventListener('click', () => { currentLevel = (currentLevel + 1) % window.BISHOP_PATH_DATA.levels.length; startLevel(currentLevel); });
  ...
  function renderLevels() {
    const levels = window.BISHOP_PATH_DATA.levels;
    const tiers = {};
    levels.forEach(lv => {
  ...
  function startLevel(id) {
    const lv = window.BISHOP_PATH_DATA.levels[id];
    n = lv.n;
    ...
    hintsUsed = 0;
    dom.hudLevel.textContent = `${id + 1}/${window.BISHOP_PATH_DATA.levels.length}`;
    timer = 0;
    showScreen('game');
    draw();
  }
  ...
  // Check if (r2, c2) is reachable from (r1, c1) by a single bishop move
  function isBishopMove(r1, c1, r2, c2) {
    const dr = r2 - r1;
    const dc = c2 - c1;
  ...
  function tryExtendPath(r, c) {
    if (walls[r][c]) return;
    const last = playerPath[playerPath.length - 1];
    if (!isBishopMove(last[0], last[1], r, c)) {
      playSfx('error');
      return;
    }
    ...
    for (let r = 0; r < n; r++) {
      for (let c = 0; c < n; c++) {
        ...
        const isValidMove = hoverCell && !isWall && !isStart &&
          (playerPath.length === 0 ||
           isBishopMove(playerPath[playerPath.length - 1][0], playerPath[playerPath.length - 1][1], r, c))
        ...
        // Hover hint for valid bishop move
  ...
  function loadSettings() {
    try {
      const s = JSON.parse(localStorage.getItem('bishoppath_settings') || '{}');
  ...
  function saveSettings() {
    try {
      localStorage.setItem('bishoppath_settings', JSON.stringify({ music: musicEnabled, sfx: sfxEnabled }));
    } catch (e) {}
  }
  ...
  window.bishoppath = {
    ...
    getN: () => n,
    getWalls: () => walls,
    // Verifier-friendly: solve a level with a given solution path
    validateSolutionPath: function(level) {
      // Check the given level's solution_path is valid (bishop moves + reachable)
      if (!level) return false;
      const sn = level.n;
      const sw = level.walls.map(w => `${w[0]},${w[1]}`);
      ...
      for (let i = 0; i < path.length - 1; i++) {
        ...
        for (let s = 1; s < Math.abs(dr); s++) {
          const cr = r1 + sr * s;
          const cc = c1 + sc * s;
          if (swSet.has(`${cr},${cc}`)) return false;
        }
      }
      // Check path reaches goal
