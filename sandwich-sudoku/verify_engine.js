#!/usr/bin/env node
/* GENERATED in-engine verifier for sandwich-sudoku — pattern follows akari/verify_engine.js.
 ...
 * Per level: loadLevel(i); G.board = L.solution; checkWin() sets G.finished; row/col sandwich sums (digits strictly between 1 and n) must match clues.
 * Usage: node sandwich-sudoku/verify_engine.js   (cwd = repo root)
 */
const fs = require('fs');
const vm = require('vm');
...
const DRIVER = "(function(){\nlet pass=0,fail=0,fails=[],failIdx=[];\nfor(let i=0;i<LEVELS.length;i++){const L=LEVELS[i];let why='';\n try{\n  loadLevel(i);\n  G.board=L.solution.map(function(r){return r.slice();});\n  let givensOK=true,cluesOK=true;\n  for(let r=0;r<L.n;r++)for(let c=0;c<L.n;c++){if(L.puzzle[r][c]&&L.puzzle[r][c]!==L.solution[r][c]){givensOK=false;why='given@'+r+','+c;}}\n  function sandwichSum(line){let s=0,started=false;for(let k=0;k<line.length;k++){const v=line[k];if(v===1||v===L.n){if(started)break;started=true;}else if(started)s+=v;}return s;}\n  for(let r=0;r<L.n;r++){if(sandwichSum(L.solution[r])!==L.row_clues[r]){cluesOK=false;why='rowClue'+r;}}\n  for(let c=0;c<L.n;c++){const col=[];for(let r=0;r<L.n;r++)col.push(L.solution[r][c]);if(sandwichSum(col)!==L.col_clues[c]){cluesOK=false;why='colClue'+c;}}\n  checkWin();\n  if(G.finished===true&&givensOK&&cluesOK)pass++;else{fail++;failIdx.push(i+1);fails.push('L'+(i+1)+(G.finished===true?'':' noWin')+' '+why);}\n }catch(e){fail++;failIdx.push(i+1);fails.push('L'+(i+1)+' EX:'+String(e.message).slice(0,60));}\n}\nreturn {pass:pass,fail:fail,total:LEVELS.length,failIdx:failIdx,fails:fails.slice(0,15),verdict:fail===0?'PASS':'FAIL'};\n})()";
...
console.log('sandwich-sudoku in-engine verification: ' + out.pass + '/' + out.total + ' levels, verdict=' + out.verdict);
