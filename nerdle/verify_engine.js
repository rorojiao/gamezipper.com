#!/usr/bin/env node
/* GENERATED in-engine verifier for nerdle — pattern follows akari/verify_engine.js.
 ...
 * Per level: startLevel(i); G.currentGuess = L.ans; submitGuess() must set G.won (and isValidEquation(L.ans) must hold).
 * Usage: node nerdle/verify_engine.js   (cwd = repo root)
 */
const fs = require('fs');
const vm = require('vm');
...
console.log('nerdle in-engine verification: ' + out.pass + '/' + out.total + ' levels, verdict=' + out.verdict);
