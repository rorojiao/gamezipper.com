#!/usr/bin/env node
/* meowdoku — permanent alias page redirecting to star-battle (no engine by design).
...
T('refresh-target', last(refresh) === 'star-battle', refresh);
T('js-target', last(js) === 'star-battle', js);
T('canonical-target', last(canon) === 'star-battle', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'star-battle', 'index.html');
...
T('target-is-real-game', tok, 'star-battle/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'star-battle' } };
