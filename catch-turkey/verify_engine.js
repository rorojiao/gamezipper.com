#!/usr/bin/env node
/* catch-turkey — permanent alias page redirecting to whack-a-mole (no engine by design).
...
T('refresh-target', last(refresh) === 'whack-a-mole', refresh);
T('js-target', last(js) === 'whack-a-mole', js);
T('canonical-target', last(canon) === 'whack-a-mole', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'whack-a-mole', 'index.html');
...
T('target-is-real-game', tok, 'whack-a-mole/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'whack-a-mole' } };
