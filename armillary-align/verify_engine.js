#!/usr/bin/env node
/* armillary-align — permanent alias page redirecting to antikythera-mechanism (no engine by design).
...
T('refresh-target', last(refresh) === 'antikythera-mechanism', refresh);
T('js-target', last(js) === 'antikythera-mechanism', js);
T('canonical-target', last(canon) === 'antikythera-mechanism', canon);
...
T('target-is-real-game', tok, 'antikythera-mechanism/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'antikythera-mechanism' } };
