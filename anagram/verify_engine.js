#!/usr/bin/env node
/* anagram — permanent alias page redirecting to text-twist (no engine by design).
...
T('refresh-target', last(refresh) === 'text-twist', refresh);
T('js-target', last(js) === 'text-twist', js);
T('canonical-target', last(canon) === 'text-twist', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'text-twist', 'index.html');
...
T('target-is-real-game', tok, 'text-twist/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'text-twist' } };
