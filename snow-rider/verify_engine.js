#!/usr/bin/env node
/* snow-rider — permanent alias page redirecting to slope (no engine by design).
...
T('refresh-target', last(refresh) === 'slope', refresh);
T('js-target', last(js) === 'slope', js);
T('canonical-target', last(canon) === 'slope', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'slope', 'index.html');
...
T('target-is-real-game', tok, 'slope/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'slope' } };
