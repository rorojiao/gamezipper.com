#!/usr/bin/env node
/* GENERATED in-engine verifier for four-pics-one-word — pattern follows akari/verify_engine.js.
 ...
 * IIFE-wrapped engine: surgery exports {game,LEVELS} after `const game=new Game()` and win flag in onCorrect. Per level: fill answerSlots with L.answer letters; checkAnswer() -> onCorrect.
 * Usage: node four-pics-one-word/verify_engine.js   (cwd = repo root)
 */
const fs = require('fs');
const vm = require('vm');
...
const SURGERY = [
 [
  "const game=new Game();",
  "const game=new Game();window.__VE_4p1w={game:game,LEVELS:LEVELS};"
 ],
 [
  "onCorrect(){",
  "onCorrect(){window.__WON=true;"
 ]
];
...
const DRIVER = "(function(){\nconst VE=window.__VE_4p1w;let pass=0,fail=0,fails=[],failIdx=[];\nfor(let i=0;i<VE.LEVELS.length;i++){const L=VE.LEVELS[i];\n try{\n  const g=VE.game;\n  if(g.state==='daily')g.state='levels';\n  g.currentLevelIdx=i;\n  g.answerSlots=new Array(L.answer.length).fill(null);\n  g.revealedPositions=new Set();\n  try{g.layoutLevel();}catch(eL){}\n  g.answerSlots=L.answer.split('').map(function(ch){return {letter:ch,used:true,removed:false};});\n  window.__WON=false;\n  g.checkAnswer();\n  if(window.__WON===true)pass++;else{fail++;failIdx.push(i+1);fails.push('L'+(i+1)+' noWin');}\n }catch(e){fail++;failIdx.push(i+1);fails.push('L'+(i+1)+' EX:'+String(e.message).slice(0,60));}\n}\nreturn {pass:pass,fail:fail,total:VE.LEVELS.length,failIdx:failIdx,fails:fails.slice(0,15),verdict:fail===0?'PASS':'FAIL'};\n})()";
...
console.log('four-pics-one-word in-engine verification: ' + out.pass + '/' + out.total + ' levels, verdict=' + out.verdict);
