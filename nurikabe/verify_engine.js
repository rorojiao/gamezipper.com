#!/usr/bin/env node
/* GENERATED in-engine verifier for nurikabe — pattern follows akari/verify_engine.js.
 ...
 * IIFE-wrapped engine: surgery exports {PUZZLES,state,startLevel,checkWin,verifyPuzzle}, win flag in onWin. Per level: startLevel(i); grid from sol bitstring; checkWin -> onWin; engine verifyPuzzle(p) must be true.
 * Usage: node nurikabe/verify_engine.js   (cwd = repo root)
 */
const fs = require('fs');
const vm = require('vm');
...
const SURGERY = [
 [
  "function checkWin(){",
  "window.__VE_nuri=function(){return {PUZZLES:PUZZLES,LEVELS:LEVELS,state:state,startLevel:startLevel,checkWin:checkWin,verifyPuzzle:verifyPuzzle};};\nfunction checkWin(){"
 ],
 [
  "function onWin(){",
  "function onWin(){window.__WON=true;"
 ]
];
for (const [find, repl] of SURGERY) {
  if (!code.includes(find)) { console.error('surgery anchor missing: ' + JSON.stringify(find.slice(0, 50))); process.exit(1); }
...
const DRIVER = "(function(){\nconst VE=window.__VE_nuri;let pass=0,fail=0,fails=[],failIdx=[];\nconst PUZ=VE().PUZZLES;\nfor(let i=0;i<PUZ.length;i++){const p=PUZ[i];\n try{\n  const preValid=VE().verifyPuzzle(p)===true;\n  VE().startLevel(i);\n  const st=VE().state;\n  for(let r=0;r<p.rows;r++)for(let c=0;c<p.cols;c++){const key=r+','+c;if(st.clues[key]!==undefined)continue;\n   const sv=parseInt(p.sol[r*p.cols+c]);st.cell[key]=sv===1?1:2;}\n  window.__WON=false;\n  VE().checkWin();\n  if(window.__WON===true&&preValid)pass++;else{fail++;failIdx.push(i+1);fails.push('L'+(i+1)+(window.__WON===true?'':' noWin')+(!preValid?' verifyPuzzle(false)':''));}\n }catch(e){fail++;failIdx.push(i+1);fails.push('L'+(i+1)+' EX:'+String(e.message).slice(0,60));}\n}\nreturn {pass:pass,fail:fail,total:PUZ.length,failIdx:failIdx,fails:fails.slice(0,15),verdict:fail===0?'PASS':'FAIL'};\n})()";
...
console.log('nurikabe in-engine verification: ' + out.pass + '/' + out.total + ' levels, verdict=' + out.verdict);
