#!/usr/bin/env node
/* pixel-logic — permanent alias page redirecting to nonogram (no engine by design).
...
T('refresh-target', last(refresh) === 'nonogram', refresh);
T('js-target', last(js) === 'nonogram', js);
T('canonical-target', last(canon) === 'nonogram', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'nonogram', 'index.html');
...
T('target-is-real-game', tok, 'nonogram/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'nonogram' } };
