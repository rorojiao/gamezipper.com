#!/usr/bin/env node
/* hole-stars — permanent alias page redirecting to black-hole (no engine by design).
...
T('refresh-target', last(refresh) === 'black-hole', refresh);
T('js-target', last(js) === 'black-hole', js);
T('canonical-target', last(canon) === 'black-hole', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'black-hole', 'index.html');
...
T('target-is-real-game', tok, 'black-hole/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'black-hole' } };
