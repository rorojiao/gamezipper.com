#!/usr/bin/env node
/* number-slide — permanent alias page redirecting to sliding-puzzle (no engine by design).
...
T('refresh-target', last(refresh) === 'sliding-puzzle', refresh);
T('js-target', last(js) === 'sliding-puzzle', js);
T('canonical-target', last(canon) === 'sliding-puzzle', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'sliding-puzzle', 'index.html');
...
T('target-is-real-game', tok, 'sliding-puzzle/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'sliding-puzzle' } };
