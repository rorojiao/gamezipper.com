// Camel Path game engine
//
// Rules:
// - N x N grid with walls (impassable cells).
// - Player draws a path from START to GOAL using CAMEL moves only:
//   (1,3) leaper — 8 directions. Each move is a single hop (not a slide).
//   (|dr|, |dc|) = (1, 3) or (3, 1).
...
//
// Controls:
// - Click/tap any cell to extend the path. Cells reachable by a camel hop
...
(function() {
  ...
  const SAVE_KEY = 'camelpath_save_v1';
  const HINT_LIMIT = 3;
  ...
  function bindEvents() {
    ...
    if (dom['btn-play']) dom['btn-play'].addEventListener('click', () => {
      try { renderLevels(); } catch(e) { console.error('[camelpath] renderLevels:', e.message); }
      try { showScreen('levels'); } catch(e) { console.error('[camelpath] showScreen:', e.message); }
    });
    ...
    if (dom['btn-next']) dom['btn-next'].addEventListener('click', () => {
      if (currentLevel + 1 < window.CAMEL_PATH_DATA.levels.length) {
        currentLevel++;
  ...
  function renderLevels() {
    const levels = window.CAMEL_PATH_DATA.levels;
    const tiers = {};
    ...
    tierOrder.forEach(tier => {
      ...
      html += `<div class="tier-section"><div class="tier-title" style="color:${TIER_COLORS[tier] || '#64ffda'}">${tier}</div><div class="level-grid">`;
  ...
  function startLevel(id) {
    const lv = window.CAMEL_PATH_DATA.levels[id];
    n = lv.n;
    ...
    hintsUsed = 0;
    dom['hudLevel'].textContent = `${id + 1}/${window.CAMEL_PATH_DATA.levels.length}`;
    timer = 0;
    showScreen('game');
    dom['winOverlay'].classList.add('hidden');
    resizeCanvas();
  ...
  // Check if (r2, c2) is reachable from (r1, c1) by a single camel hop
  function isCamelMove(r1, c1, r2, c2) {
    const dr = r2 - r1;
    const dc = c2 - c1;
  ...
  function tryExtendPath(r, c) {
    if (walls[r][c]) return;
    const last = playerPath[playerPath.length - 1];
    if (!isCamelMove(last[0], last[1], r, c)) {
      playSfx('error');
      return;
    }
    ...
    for (let r = 0; r < n; r++) {
      for (let c = 0; c < n; c++) {
        ...
        const isValidMove = hoverCell && !isWall && !isStart &&
          (playerPath.length === 0 ||
           isCamelMove(playerPath[playerPath.length - 1][0], playerPath[playerPath.length - 1][1], r, c))
        ...
        // Hover hint for valid camel move
  ...
  function loadSettings() {
    try {
      const s = JSON.parse(localStorage.getItem('camelpath_settings') || '{}');
  ...
  function saveSettings() {
    try {
      localStorage.setItem('camelpath_settings', JSON.stringify({ music: musicEnabled, sfx: sfxEnabled }));
    } catch (e) {}
  }
  ...
  window.camelpath = {
    ...
    getN: () => n,
    getWalls: () => walls,
    // Verifier-friendly: validate a given solution path
    ...
    // Validator hook: solve level with provided solution path
