#!/usr/bin/env node
/* jelly-dye — permanent alias page redirecting to flood-fill (no engine by design).
...
T('refresh-target', last(refresh) === 'flood-fill', refresh);
T('js-target', last(js) === 'flood-fill', js);
T('canonical-target', last(canon) === 'flood-fill', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'flood-fill', 'index.html');
...
T('target-is-real-game', tok, 'flood-fill/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'flood-fill' } };
