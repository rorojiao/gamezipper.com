#!/usr/bin/env node
/* GENERATED in-engine verifier for magic-square — pattern follows akari/verify_engine.js.
 ...
 * IIFE-wrapped engine: surgery exports {LEVELS,state,loadLevel,checkWin}, win flag in winLevel. Per level: loadLevel(i); blanks filled from L.solution; checkWin -> winLevel.
 * Usage: node magic-square/verify_engine.js   (cwd = repo root)
 */
const fs = require('fs');
const vm = require('vm');
...
const SURGERY = [
 [
  "function checkWin(){",
  "window.__VE_msq=function(){return {LEVELS:LEVELS,state:state,loadLevel:loadLevel,checkWin:checkWin};};\nfunction checkWin(){"
 ],
 [
  "function winLevel(){",
  "function winLevel(){window.__WON=true;"
 ]
];
...
console.log('magic-square in-engine verification: ' + out.pass + '/' + out.total + ' levels, verdict=' + out.verdict);
