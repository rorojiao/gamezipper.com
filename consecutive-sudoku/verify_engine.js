#!/usr/bin/env node
/* consecutive-sudoku — permanent alias page redirecting to kropki-sudoku (no engine by design).
...
T('refresh-target', last(refresh) === 'kropki-sudoku', refresh);
T('js-target', last(js) === 'kropki-sudoku', js);
T('canonical-target', last(canon) === 'kropki-sudoku', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'kropki-sudoku', 'index.html');
...
T('target-is-real-game', tok, 'kropki-sudoku/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'kropki-sudoku' } };
