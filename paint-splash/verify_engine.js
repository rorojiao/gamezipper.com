#!/usr/bin/env node
/* paint-splash — permanent alias page redirecting to color-by-number (no engine by design).
...
T('refresh-target', last(refresh) === 'color-by-number', refresh);
T('js-target', last(js) === 'color-by-number', js);
T('canonical-target', last(canon) === 'color-by-number', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'color-by-number', 'index.html');
...
T('target-is-real-game', tok, 'color-by-number/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'color-by-number' } };
