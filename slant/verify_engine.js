#!/usr/bin/env node
/* slant — permanent alias page redirecting to gokigen-naname (no engine by design).
...
T('refresh-target', last(refresh) === 'gokigen-naname', refresh);
T('js-target', last(js) === 'gokigen-naname', js);
T('canonical-target', last(canon) === 'gokigen-naname', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'gokigen-naname', 'index.html');
...
T('target-is-real-game', tok, 'gokigen-naname/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'gokigen-naname' } };
