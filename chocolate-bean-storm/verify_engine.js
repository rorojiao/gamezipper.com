#!/usr/bin/env node
/* chocolate-bean-storm — permanent alias page redirecting to bubble-shooter (no engine by design).
...
T('refresh-target', last(refresh) === 'bubble-shooter', refresh);
T('js-target', last(js) === 'bubble-shooter', js);
T('canonical-target', last(canon) === 'bubble-shooter', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'bubble-shooter', 'index.html');
...
T('target-is-real-game', tok, 'bubble-shooter/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'bubble-shooter' } };
