// Zebra Path game engine
//
// Rules:
// - N x N grid with walls (impassable cells).
// - Player draws a path from START to GOAL using ZEBRA moves only:
//   (2,3) leaper — 8 directions. Each move is a single hop (not a slide).
//   (|dr|, |dc|) = (2, 3) or (3, 2).
...
//
// Controls:
// - Click/tap any cell to extend the path. Cells reachable by a zebra hop
...
(function() {
  ...
  const SAVE_KEY = 'zebrapath_save_v1';
  const HINT_LIMIT = 3;
  ...
  function bindEvents() {
    if (dom['btn-play']) dom['btn-play'].addEventListener('click', () => {
      try { renderLevels(); } catch(e) { console.error('[zebrapath] renderLevels:', e.message); }
      try { showScreen('levels'); } catch(e) { console.error('[zebrapath] showScreen:', e.message); }
    });
    ...
    if (dom['btn-next']) dom['btn-next'].addEventListener('click', () => {
      if (currentLevel + 1 < window.ZEBRA_PATH_DATA.levels.length) {
        currentLevel++;
  ...
  function renderLevels() {
    const levels = window.ZEBRA_PATH_DATA.levels;
    const tiers = {};
    ...
    tierOrder.forEach(tier => {
      ...
      html += `<div class="tier-section"><div class="tier-title" style="color:${TIER_COLORS[tier] || '#a3e635'}">${tier}</div><div class="level-grid">`;
  ...
  function startLevel(id) {
    const lv = window.ZEBRA_PATH_DATA.levels[id];
    n = lv.n;
    ...
    hintsUsed = 0;
    dom['hudLevel'].textContent = `${id + 1}/${window.ZEBRA_PATH_DATA.levels.length}`;
    timer = 0;
    showScreen('game');
    dom['winOverlay'].classList.add('hidden');
    resizeCanvas();
  ...
  function spawnConfetti() {
    ...
    const colors = ['#a3e635', '#22d3ee', '#fbbf24', '#f43f5e'];
    const N = 80;
    const pieces = [];
    for (let i = 0; i < N; i++) {
      pieces.push({
    ...
    function loop(now) {
      ...
      ctx.save();
      ctx.fillStyle = 'rgba(8,16,28,0.2)';
  ...
  // Check if (r2, c2) is reachable from (r1, c1) by a single zebra hop
  function isZebraMove(r1, c1, r2, c2) {
    const dr = r2 - r1;
    const dc = c2 - c1;
    const sorted = [Math.abs(dr), Math.abs(dc)].sort((a, b) => a - b);
    return sorted[0] === 2 && sorted[1] === 3;
  }

  function tryExtendPath(r, c) {
    if (walls[r][c]) return;
    const last = playerPath[playerPath.length - 1];
    if (!isZebraMove(last[0], last[1], r, c)) {
      playSfx('error');
      return;
    }
  ...
  function draw() {
    ...
    for (let r = 0; r < n; r++) {
      for (let c = 0; c < n; c++) {
        ...
        const isValidMove = hoverCell && !isWall && !isStart &&
          (playerPath.length === 0 ||
           isZebraMove(playerPath[playerPath.length - 1][0], playerPath[playerPath.length - 1][1], r, c))
        ...
        if (isWall) {
          ...
          ctx.fillStyle = inPath ? '#1a2d30' : '#0c1828';
          ctx.fillRect(x - cs * 0.45, y - cs * 0.45, cs * 0.9, cs * 0.9);
        }
    ...
    for (let i = 0; i < playerPath.length; i++) {
      ...
      const x = margin + c * cs;
      const y = margin + r * cs;
      ctx.fillStyle = i === 0 ? '#22d3ee' : (i === playerPath.length - 1 ? '#fbbf24' : '#a3e635');
      ctx.beginPath();
  ...
  function playBgMusic() {
    ...
    if (!ctx) return;
    if (ctx.state === 'suspended') ctx.resume();
    const notes = [392.0, 440.0, 523.25, 587.33, 659.25, 523.25, 440.0, 392.0];
    const step = 0.5;
    notes.forEach((freq, i) => {
  ...
  function loadSettings() {
    try {
      const s = JSON.parse(localStorage.getItem('zebrapath_settings') || '{}');
  ...
  function saveSettings() {
    try {
      localStorage.setItem('zebrapath_settings', JSON.stringify({ music: musicEnabled, sfx: sfxEnabled }));
    } catch (e) {}
  }
  ...
  window.zebrapath = {
    ...
    validateSolutionPath: function(level) {
      ...
      for (let i = 0; i < path.length - 1; i++) {
        ...
        if (sorted[0] !== 2 || sorted[1] !== 3) return false;
