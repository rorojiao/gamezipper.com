#!/usr/bin/env node
/* ball-catch — permanent alias page redirecting to brick-breaker (no engine by design).
...
T('refresh-target', last(refresh) === 'brick-breaker', refresh);
T('js-target', last(js) === 'brick-breaker', js);
T('canonical-target', last(canon) === 'brick-breaker', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'brick-breaker', 'index.html');
...
T('target-is-real-game', tok, 'brick-breaker/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'brick-breaker' } };
