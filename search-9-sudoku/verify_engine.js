#!/usr/bin/env node
/* search-9-sudoku — permanent alias page redirecting to outside-sudoku (no engine by design).
...
T('refresh-target', last(refresh) === 'outside-sudoku', refresh);
T('js-target', last(js) === 'outside-sudoku', js);
T('canonical-target', last(canon) === 'outside-sudoku', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'outside-sudoku', 'index.html');
...
T('target-is-real-game', tok, 'outside-sudoku/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'outside-sudoku' } };
