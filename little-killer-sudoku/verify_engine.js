#!/usr/bin/env node
/* little-killer-sudoku — permanent alias page redirecting to killer-sudoku (no engine by design).
...
T('refresh-target', last(refresh) === 'killer-sudoku', refresh);
T('js-target', last(js) === 'killer-sudoku', js);
T('canonical-target', last(canon) === 'killer-sudoku', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'killer-sudoku', 'index.html');
...
T('target-is-real-game', tok, 'killer-sudoku/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'killer-sudoku' } };
