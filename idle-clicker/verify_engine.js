#!/usr/bin/env node
/* idle-clicker — permanent alias page redirecting to cookie-clicker (no engine by design).
...
T('refresh-target', last(refresh) === 'cookie-clicker', refresh);
T('js-target', last(js) === 'cookie-clicker', js);
T('canonical-target', last(canon) === 'cookie-clicker', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'cookie-clicker', 'index.html');
...
T('target-is-real-game', tok, 'cookie-clicker/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'cookie-clicker' } };
