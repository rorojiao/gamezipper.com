#!/usr/bin/env node
/* word-puzzle — permanent alias page redirecting to wordle (no engine by design).
...
T('refresh-target', last(refresh) === 'wordle', refresh);
T('js-target', last(js) === 'wordle', js);
T('canonical-target', last(canon) === 'wordle', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'wordle', 'index.html');
...
T('target-is-real-game', tok, 'wordle/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'wordle' } };
