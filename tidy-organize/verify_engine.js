#!/usr/bin/env node
/* tidy-organize — permanent alias page redirecting to unpacking (no engine by design).
...
T('refresh-target', last(refresh) === 'unpacking', refresh);
T('js-target', last(js) === 'unpacking', js);
T('canonical-target', last(canon) === 'unpacking', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'unpacking', 'index.html');
...
T('target-is-real-game', tok, 'unpacking/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'unpacking' } };
