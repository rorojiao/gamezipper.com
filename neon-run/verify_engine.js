#!/usr/bin/env node
/* neon-run — permanent alias page redirecting to neon-dash (no engine by design).
...
T('refresh-target', last(refresh) === 'neon-dash', refresh);
T('js-target', last(js) === 'neon-dash', js);
T('canonical-target', last(canon) === 'neon-dash', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'neon-dash', 'index.html');
...
T('target-is-real-game', tok, 'neon-dash/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'neon-dash' } };
