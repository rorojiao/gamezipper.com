#!/usr/bin/env node
/* arrow-block-escape — permanent alias page redirecting to tap-away (no engine by design).
...
T('refresh-target', last(refresh) === 'tap-away', refresh);
T('js-target', last(js) === 'tap-away', js);
T('canonical-target', last(canon) === 'tap-away', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'tap-away', 'index.html');
...
T('target-is-real-game', tok, 'tap-away/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'tap-away' } };
