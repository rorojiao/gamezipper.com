#!/usr/bin/env node
/* tripeaks-solitaire — permanent alias page redirecting to tripeaks (no engine by design).
...
T('refresh-target', last(refresh) === 'tripeaks', refresh);
T('js-target', last(js) === 'tripeaks', js);
T('canonical-target', last(canon) === 'tripeaks', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'tripeaks', 'index.html');
...
T('target-is-real-game', tok, 'tripeaks/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'tripeaks' } };
