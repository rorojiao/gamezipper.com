#!/usr/bin/env node
/* mahjong-connect — permanent alias page redirecting to onet (no engine by design).
...
T('refresh-target', last(refresh) === 'onet', refresh);
T('js-target', last(js) === 'onet', js);
T('canonical-target', last(canon) === 'onet', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'onet', 'index.html');
...
T('target-is-real-game', tok, 'onet/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'onet' } };
