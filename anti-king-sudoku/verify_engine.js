#!/usr/bin/env node
/* anti-king-sudoku — permanent alias page redirecting to anti-knight-sudoku (no engine by design).
...
T('refresh-target', last(refresh) === 'anti-knight-sudoku', refresh);
T('js-target', last(js) === 'anti-knight-sudoku', js);
T('canonical-target', last(canon) === 'anti-knight-sudoku', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'anti-knight-sudoku', 'index.html');
...
T('target-is-real-game', tok, 'anti-knight-sudoku/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'anti-knight-sudoku' } };
