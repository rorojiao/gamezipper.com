...
function main() {
  ...
  console.log('=== Tatami QA Checklist ===\n');
  ...
  t('5. <title>Tatami', html.includes('<title>Tatami'));
  ...
  t('20. canonical points to /tatami/', html.includes('https://gamezipper.com/tatami/'));
  ...
  t('50. pendingStart 2-click placement', gameJs.includes('pendingStart'));
  ...
  t('75. data.js valid JS', dataJs.startsWith('const TATAMI_LEVELS'));
  t('76. data.js includes levels array', dataJs.includes('"levels"'));
  t('77. All solutions non-empty', levelsJson.levels.every(l => l.solution_tiling.length > 0));
  ...
  // Bonus (3)
  console.log('\n-- Bonus --');
  t('83. JSON-LD parses', JSON.parse(html.match(/<script type="application\/ld\+json">[^<]+VideoGame[^<]+<\/script>/)[0].replace(/<[^>]+>/g, '')).name === 'Tatami');
  t('84. FAQ has 4 Q&A', (html.match(/"@type":"Question"/g) || []).length >= 4);
  t('85. Breadcrumb has 2 items', (html.match(/"@type":"ListItem"/g) || []).length === 2);

  console.log(`\n=== Tatami QA: ${passed}/${total} PASS, ${failed} FAIL ===`);
  if (failed > 0) process.exit(1);
}

function SAVE_KEY() { return 'tatami_save_v1'; }
