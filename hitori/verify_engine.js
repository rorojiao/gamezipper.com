#!/usr/bin/env node
/* GENERATED in-engine verifier for hitori — pattern follows akari/verify_engine.js.
 ...
 * IIFE-wrapped engine: source surgery exports {LEVELS,state,startLevel,checkWin} and win flag in onWin. Per level: startLevel(i); state.shades = L.solution; checkWin -> onWin.
 * Usage: node hitori/verify_engine.js   (cwd = repo root)
 */
const fs = require('fs');
const vm = require('vm');
...
const SURGERY = [
 [
  "function checkWin(){",
  "window.__VE_hitori=function(){return {LEVELS:LEVELS,state:state,startLevel:startLevel,checkWin:checkWin,checkConflicts:checkConflicts,checkAdjacent:checkAdjacent};};\nfunction checkWin(){"
 ],
 [
  "function onWin(){",
...
console.log('hitori in-engine verification: ' + out.pass + '/' + out.total + ' levels, verdict=' + out.verdict);
