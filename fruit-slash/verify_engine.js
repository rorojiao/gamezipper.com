#!/usr/bin/env node
/* fruit-slash — permanent alias page redirecting to slice-master (no engine by design).
...
T('refresh-target', last(refresh) === 'slice-master', refresh);
T('js-target', last(js) === 'slice-master', js);
T('canonical-target', last(canon) === 'slice-master', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'slice-master', 'index.html');
...
T('target-is-real-game', tok, 'slice-master/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'slice-master' } };
