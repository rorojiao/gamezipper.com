#!/usr/bin/env node
/* word-cookies — permanent alias page redirecting to wordscapes (no engine by design).
...
T('refresh-target', last(refresh) === 'wordscapes', refresh);
T('js-target', last(js) === 'wordscapes', js);
T('canonical-target', last(canon) === 'wordscapes', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'wordscapes', 'index.html');
...
T('target-is-real-game', tok, 'wordscapes/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'wordscapes' } };
