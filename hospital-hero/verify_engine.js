#!/usr/bin/env node
/* hospital-hero — permanent alias page redirecting to hotel-rush (no engine by design).
...
T('refresh-target', last(refresh) === 'hotel-rush', refresh);
T('js-target', last(js) === 'hotel-rush', js);
T('canonical-target', last(canon) === 'hotel-rush', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'hotel-rush', 'index.html');
...
T('target-is-real-game', tok, 'hotel-rush/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'hotel-rush' } };
