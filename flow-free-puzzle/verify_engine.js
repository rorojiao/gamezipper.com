...
const failures = requirements.filter(([, pattern]) => !pattern.test(html)).map(([name]) => name);
...
console.log(`Redirect contract PASS: ${requirements.length} checks for ${destination}`);
