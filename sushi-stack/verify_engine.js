#!/usr/bin/env node
/* sushi-stack — permanent alias page redirecting to tower-stacker-3d (no engine by design).
...
T('refresh-target', last(refresh) === 'tower-stacker-3d', refresh);
T('js-target', last(js) === 'tower-stacker-3d', js);
T('canonical-target', last(canon) === 'tower-stacker-3d', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'tower-stacker-3d', 'index.html');
...
T('target-is-real-game', tok, 'tower-stacker-3d/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'tower-stacker-3d' } };
