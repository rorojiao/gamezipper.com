#!/usr/bin/env python3
"""R622 — Add 2nd AdSense slot (7373732357) to 4 high-signal zero-fill (PV>=8) game pages.
...
Usage:
  python3 scripts/r622-add-second-adsense-slot.py --dry-run
  python3 scripts/r622-add-second-adsense-slot.py
"""
...
PAGES = [
    "recoil", "monster-truck-madness", "gecko-out", "coin-pusher",
]
...
R622_BLOCK = (
    '<!-- ADS-OPT 2026-08-31 R622: 2nd AdSense slot (data-ad-slot="737...") on 4 high-signal zero-fill (PV>=8) game pages. '
    'BI 7d (2026-08-31): 12 of 12 pages had 0 fills despite legacy data-ad-slot="109..." in HTML; '
    'PV: recoil=10, monster-truck-madness=9, gecko-out=9, coin-pusher=8, total=36 PV/7d. R621 cohort proved blog SBF/PV=1.0-1.1 within hours; closing last 4 high-PV zero-fill game gaps.. Pattern: parallel div after gz-ad-below-game close, '
    'R383 CLS-safe wrapper (same as R548/R549/R550/R554). -->'
    '<div id="gz-ad-r622" style="position:relative;min-height:100px;max-height:280px;'
...
def process_page(page: str, dry_run: bool = False) -> tuple[str, str]:
    ...
    if "gz-ad-r622" in html:
        return ("skipped", "already has gz-ad-r622 marker (R622 idempotent)")
    ...
    new_html = html[:insert_at] + R622_BLOCK + html[insert_at:]
    ...
    # Verify gz-ad-r622 is NOT inside a comment
    r622_pos = new_html.find('gz-ad-r622')
    ...
    if r622_ins_block_pos != -1:
        ...
        if between.count('-->') == 0:
            return ("error", "gz-ad-r622 landed inside an HTML comment — abort")

    if dry_run:
        return ("applied-dryrun", f"{len(R622_BLOCK)} chars at offset {insert_at}; char_after={char_after!r}")

    p.write_text(new_html)
    return ("applied", f"inserted {len(R622_BLOCK)} chars after gz-ad-below-game close at offset {insert_at}")
