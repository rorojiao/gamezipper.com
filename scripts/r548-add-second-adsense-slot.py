#!/usr/bin/env python3
"""
R548 — 2nd AdSense slot (7373732357) on 6 zero-fill game pages.
...
R548 picks (sorted by 7d PV desc, all 737=0 in HTML):
  1. /slice-master/         PV=18
  2. /laser-maze/           PV=18
  3. /save-the-doge/        PV=16
  4. /hourglass-swap/       PV=12
  5. /gravity-flip/         PV=12
  6. /count-master/         PV=12

Total: 86 PV/7d. Pattern (R532 sibling): parallel div after gz-ad-below-game close,
R383 CLS-safe wrapper. Idempotent: skips if 737 already present.
...
def main():
    ...
    for rel_path in PAGES:
        ...
        if insert_at is None:
            ...
            continue
        new_content = content[:insert_at] + R548_INSERT + content[insert_at:]
        with open(fp, "w") as f:
            f.write(new_content)
        with open(fp) as f:
            verify = f.read()
        ...
        has_marker = "gz-ad-r548" in verify
        ins_109_pos = verify.find("1099212472")
        r548_pos = verify.find("gz-ad-r548")
    ...
    print()
    print(f"=== R548 summary: {len(inserted)} inserted, {len(skipped)} skipped, {len(failed)} failed ===")
