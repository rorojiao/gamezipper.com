#!/usr/bin/env python3
"""R559 — Add 2nd AdSense slot (7373732357) to 12 zero-fill game pages.
...
Usage:
  python3 scripts/r559-add-second-adsense-slot.py --dry-run
  python3 scripts/r559-add-second-adsense-slot.py
"""
...
PAGES = [
    "cloud-sheep", "spelling-bee", "number-nexus", "crossmath",
    "stained-glass", "nail-art", "usotatami", "shikaku",
    "pin-master", "petal-path", "numbrix", "hexa-sort",
]
...
R559_BLOCK = (
    '<!-- ADS-OPT 2026-08-30 R559: 2nd AdSense slot (data-ad-slot="737...") on 12 zero-fill game pages. '
    'BI 7d (2026-08-30): 12 of 12 pages had 0 fills despite legacy data-ad-slot="109..." in HTML; '
    ...
    '<div id="gz-ad-r559" style="position:relative;min-height:100px;max-height:280px;'
...
def process_page(page: str, dry_run: bool = False) -> tuple[str, str]:
    ...
    if "gz-ad-r559" in html:
        return ("skipped", "already has gz-ad-r559 marker (R559 idempotent)")
    ...
    new_html = html[:insert_at] + R559_BLOCK + html[insert_at:]
    ...
    # Verify gz-ad-r559 is NOT inside a comment
    r559_pos = new_html.find('gz-ad-r559')
    r559_ins_block_pos = new_html.rfind('<!--', 0, r559_pos)
    if r559_ins_block_pos != -1:
        # Check if there's a --> between r559_ins_block_pos and r559_pos
        between = new_html[r559_ins_block_pos:r559_pos]
        if between.count('-->') == 0:
            return ("error", "gz-ad-r559 landed inside an HTML comment — abort")

    if dry_run:
        return ("applied-dryrun", f"{len(R559_BLOCK)} chars at offset {insert_at}; char_after={char_after!r}")

    p.write_text(new_html)
    return ("applied", f"inserted {len(R559_BLOCK)} chars after gz-ad-below-game close at offset {insert_at}")
