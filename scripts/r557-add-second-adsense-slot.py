#!/usr/bin/env python3
"""R557 — Add 2nd AdSense slot (7373732357) to 12 zero-fill game pages.
...
Usage:
  python3 scripts/r557-add-second-adsense-slot.py --dry-run
  python3 scripts/r557-add-second-adsense-slot.py
"""
...
PAGES = [
    "plug-master", "trebuchet-trajectory", "string-art", "ricochet-robots",
    "hexxagon", "color-switch", "conveyor-sort", "toichika", "slide-cat",
    "knotwords", "dice-merge", "constellation-connect",
]
...
R557_BLOCK = (
    '<!-- ADS-OPT 2026-08-29 R557: 2nd AdSense slot (data-ad-slot="737...") on 12 zero-fill game pages. '
    'BI 7d (2026-08-29): 12 of 12 pages had 0 fills despite legacy data-ad-slot="109..." in HTML; '
    'PV ranges 8-20 (cumulative 122 PV/7d). Pattern: parallel div after gz-ad-below-game close, '
    'R383 CLS-safe wrapper (same as R548/R549/R550/R554). -->'
    '<div id="gz-ad-r557" style="position:relative;min-height:100px;max-height:280px;'
...
def process_page(page: str, dry_run: bool = False) -> tuple[str, str]:
    ...
    if "gz-ad-r557" in html:
        return ("skipped", "already has gz-ad-r557 marker (R557 idempotent)")
    ...
    new_html = html[:insert_at] + R557_BLOCK + html[insert_at:]
    ...
    # Verify gz-ad-r557 is NOT inside a comment
    r557_pos = new_html.find('gz-ad-r557')
    r557_ins_block_pos = new_html.rfind('<!--', 0, r557_pos)
    if r557_ins_block_pos != -1:
        # Check if there's a --> between r557_ins_block_pos and r557_pos
        between = new_html[r557_ins_block_pos:r557_pos]
        if between.count('-->') == 0:
            return ("error", "gz-ad-r557 landed inside an HTML comment — abort")

    if dry_run:
        return ("applied-dryrun", f"{len(R557_BLOCK)} chars at offset {insert_at}; char_after={char_after!r}")

    p.write_text(new_html)
    return ("applied", f"inserted {len(R557_BLOCK)} chars after gz-ad-below-game close at offset {insert_at}")
