#!/usr/bin/env node
/* audio-rhythm-puzzle — permanent alias page redirecting to simon-says (no engine by design).
...
T('refresh-target', last(refresh) === 'simon-says', refresh);
T('js-target', last(js) === 'simon-says', js);
T('canonical-target', last(canon) === 'simon-says', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'simon-says', 'index.html');
...
T('target-is-real-game', tok, 'simon-says/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'simon-says' } };
