#!/usr/bin/env node
/* brainrot-blocks — permanent alias page redirecting to woodoku (no engine by design).
...
T('refresh-target', last(refresh) === 'woodoku', refresh);
T('js-target', last(js) === 'woodoku', js);
T('canonical-target', last(canon) === 'woodoku', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'woodoku', 'index.html');
...
T('target-is-real-game', tok, 'woodoku/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'woodoku' } };
