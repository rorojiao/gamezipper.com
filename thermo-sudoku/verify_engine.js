#!/usr/bin/env node
/* GENERATED in-engine verifier for thermo-sudoku — pattern follows akari/verify_engine.js.
 ...
 * Per level: loadLevel(i); G.board = L.solution; checkWin() sets G.finished; thermometer paths must be strictly increasing in solution.
 * Usage: node thermo-sudoku/verify_engine.js   (cwd = repo root)
 */
const fs = require('fs');
const vm = require('vm');
...
const DRIVER = "(function(){\nlet pass=0,fail=0,fails=[],failIdx=[];\nfor(let i=0;i<LEVELS.length;i++){const L=LEVELS[i];let why='';\n try{\n  loadLevel(i);\n  G.board=L.solution.map(function(r){return r.slice();});\n  let givensOK=true,thermoOK=true;\n  for(let r=0;r<L.n;r++)for(let c=0;c<L.n;c++){if(L.givens[r][c]&&L.givens[r][c]!==L.solution[r][c]){givensOK=false;why='given@'+r+','+c;}}\n  for(let t=0;t<(L.thermos||[]).length;t++){const th=L.thermos[t];for(let k=1;k<th.length;k++){if(L.solution[th[k][0]][th[k][1]]<=L.solution[th[k-1][0]][th[k-1][1]]){thermoOK=false;why='thermo'+t+'@'+k;}}}\n  checkWin();\n  if(G.finished===true&&givensOK&&thermoOK)pass++;else{fail++;failIdx.push(i+1);fails.push('L'+(i+1)+(G.finished===true?'':' noWin')+' '+why);}\n }catch(e){fail++;failIdx.push(i+1);fails.push('L'+(i+1)+' EX:'+String(e.message).slice(0,60));}\n}\nreturn {pass:pass,fail:fail,total:LEVELS.length,failIdx:failIdx,fails:fails.slice(0,15),verdict:fail===0?'PASS':'FAIL'};\n})()";
...
console.log('thermo-sudoku in-engine verification: ' + out.pass + '/' + out.total + ' levels, verdict=' + out.verdict);
