#!/usr/bin/env node
/* GENERATED in-engine verifier for tatamibari — pattern follows akari/verify_engine.js.
 ...
 * Per level: startLevel(i); derive rectangles from L.solution region ids; place each via engine placeRegion() (regionValid gates); checkWin() must be true.
 * Usage: node tatamibari/verify_engine.js   (cwd = repo root)
 */
const fs = require('fs');
const vm = require('vm');
...
const DRIVER = "(function(){\nlet pass=0,fail=0,fails=[],failIdx=[];\nfor(let i=0;i<LEVELS.length;i++){const L=LEVELS[i];\n try{\n  startLevel(i);\n  const bb={};\n  for(let r=0;r<L.rows;r++)for(let c=0;c<L.cols;c++){const id=L.solution[r][c];\n   if(!(id in bb))bb[id]={r1:r,c1:c,r2:r,c2:c};\n   else{const b=bb[id];b.r1=Math.min(b.r1,r);b.c1=Math.min(b.c1,c);b.r2=Math.max(b.r2,r);b.c2=Math.max(b.c2,c);}}\n  const ids=Object.keys(bb).map(Number).sort(function(a,b){return a-b;});\n  let placed=true,why='';\n  for(const id of ids){if(!placeRegion(bb[id])){placed=false;why='region'+id;break;}}\n  const winOk=checkWin()===true;\n  if(placed&&winOk)pass++;else{fail++;failIdx.push(i+1);fails.push('L'+(i+1)+(placed?'':' place:'+why)+(winOk?'':' noCover'));}\n }catch(e){fail++;failIdx.push(i+1);fails.push('L'+(i+1)+' EX:'+String(e.message).slice(0,60));}\n}\nreturn {pass:pass,fail:fail,total:LEVELS.length,failIdx:failIdx,fails:fails.slice(0,15),verdict:fail===0?'PASS':'FAIL'};\n})()";
...
console.log('tatamibari in-engine verification: ' + out.pass + '/' + out.total + ' levels, verdict=' + out.verdict);
