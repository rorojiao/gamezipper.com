#!/usr/bin/env node
/* color-helix-smash — permanent alias page redirecting to helix-jump (no engine by design).
...
T('refresh-target', last(refresh) === 'helix-jump', refresh);
T('js-target', last(js) === 'helix-jump', js);
T('canonical-target', last(canon) === 'helix-jump', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'helix-jump', 'index.html');
...
T('target-is-real-game', tok, 'helix-jump/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'helix-jump' } };
