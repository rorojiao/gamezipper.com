#!/usr/bin/env node
/* dessert-blast — permanent alias page redirecting to jewel-crush (no engine by design).
...
T('refresh-target', last(refresh) === 'jewel-crush', refresh);
T('js-target', last(js) === 'jewel-crush', js);
T('canonical-target', last(canon) === 'jewel-crush', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'jewel-crush', 'index.html');
...
T('target-is-real-game', tok, 'jewel-crush/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'jewel-crush' } };
