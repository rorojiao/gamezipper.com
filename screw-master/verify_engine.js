#!/usr/bin/env node
/* screw-master — permanent alias page redirecting to pin-master (no engine by design).
...
T('refresh-target', last(refresh) === 'pin-master', refresh);
T('js-target', last(js) === 'pin-master', js);
T('canonical-target', last(canon) === 'pin-master', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'pin-master', 'index.html');
...
T('target-is-real-game', tok, 'pin-master/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'pin-master' } };
