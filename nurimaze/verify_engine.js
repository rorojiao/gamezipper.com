#!/usr/bin/env node
/* GENERATED in-engine verifier for nurimaze — pattern follows akari/verify_engine.js.
 ...
 * Per level: loadLevel(i); state.grid from state.solution (0->black,1->white); doCheck(false) must set state.won.
 * Usage: node nurimaze/verify_engine.js   (cwd = repo root)
 */
const fs = require('fs');
const vm = require('vm');
...
console.log('nurimaze in-engine verification: ' + out.pass + '/' + out.total + ' levels, verdict=' + out.verdict);
