#!/usr/bin/env node
/* triple-tile — permanent alias page redirecting to tile-master (no engine by design).
...
T('refresh-target', last(refresh) === 'tile-master', refresh);
T('js-target', last(js) === 'tile-master', js);
T('canonical-target', last(canon) === 'tile-master', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'tile-master', 'index.html');
...
T('target-is-real-game', tok, 'tile-master/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'tile-master' } };
