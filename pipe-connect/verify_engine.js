#!/usr/bin/env node
/* pipe-connect — permanent alias page redirecting to liquid-connect (no engine by design).
...
T('refresh-target', last(refresh) === 'liquid-connect', refresh);
T('js-target', last(js) === 'liquid-connect', js);
T('canonical-target', last(canon) === 'liquid-connect', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'liquid-connect', 'index.html');
...
T('target-is-real-game', tok, 'liquid-connect/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'liquid-connect' } };
