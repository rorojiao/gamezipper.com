#!/usr/bin/env node
/* GENERATED in-engine verifier for skyscrapers — pattern follows akari/verify_engine.js.
 ...
 * Per level: loadLevel(L.id); grid = L.solution on non-given cells; rule-based checkSolution(true) (Latin square + visibility clues) must pass; givens must match solution.
 * Usage: node skyscrapers/verify_engine.js   (cwd = repo root)
 */
const fs = require('fs');
const vm = require('vm');
...
const DRIVER = "(function(){\nlet pass=0,fail=0,fails=[],failIdx=[];\nfor(let i=0;i<LEVELS.length;i++){const L=LEVELS[i];\n try{\n  loadLevel(L.id,false);\n  let givensOK=true,why='';\n  for(const g of (L.givens||[])){if(L.solution[g[0]][g[1]]!==g[2]){givensOK=false;why='given@'+g[0]+','+g[1];}}\n  for(let r=0;r<L.N;r++)for(let c=0;c<L.N;c++){if(!given[r][c])grid[r][c]=L.solution[r][c];}\n  const ok=checkSolution(true)===true;\n  if(ok&&givensOK)pass++;else{fail++;failIdx.push(i+1);fails.push('L'+L.id+(ok?'':' rules')+(givensOK?'':' '+why));}\n }catch(e){fail++;failIdx.push(i+1);fails.push('L'+L.id+' EX:'+String(e.message).slice(0,60));}\n}\nreturn {pass:pass,fail:fail,total:LEVELS.length,failIdx:failIdx,fails:fails.slice(0,15),verdict:fail===0?'PASS':'FAIL'};\n})()";
...
console.log('skyscrapers in-engine verification: ' + out.pass + '/' + out.total + ' levels, verdict=' + out.verdict);
