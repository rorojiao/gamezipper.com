#!/usr/bin/env node
/* GENERATED in-engine verifier for one-line-puzzle — pattern follows akari/verify_engine.js.
 ...
 * Per level: startLevel(L.id); replay L.solution through engine addDot() (adjacency/no-revisit gates); path length must reach dots.length.
 * Usage: node one-line-puzzle/verify_engine.js   (cwd = repo root)
 */
const fs = require('fs');
const vm = require('vm');
...
const SURGERY = [
 [
  "return{showScreen,startLevel,exitGame,undoMove,resetLevel,useHint,replayLevel,nextLevel,init,cleanup};",
  "return{showScreen,startLevel,exitGame,undoMove,resetLevel,useHint,replayLevel,nextLevel,init,cleanup,addDot:addDot,getPath:function(){return path;}};"
 ]
];
...
const DRIVER = "(function(){\nlet pass=0,fail=0,fails=[],failIdx=[];\nfor(let i=0;i<LEVELS.length;i++){const L=LEVELS[i];\n try{\n  G.startLevel(L.id);\n  let allOK=true,why='';\n  for(let k=0;k<L.solution.length;k++){const ok=G.addDot(L.solution[k]);if(!ok){allOK=false;why='step'+k;break;}}\n  const winOk=G.getPath().length===L.dots.length;\n  if(allOK&&winOk)pass++;else{fail++;failIdx.push(i+1);fails.push('L'+L.id+(allOK?'':' reject@'+why)+(winOk?'':' len'));}\n }catch(e){fail++;failIdx.push(i+1);fails.push('L'+L.id+' EX:'+String(e.message).slice(0,60));}\n}\nreturn {pass:pass,fail:fail,total:LEVELS.length,failIdx:failIdx,fails:fails.slice(0,15),verdict:fail===0?'PASS':'FAIL'};\n})()";
...
console.log('one-line-puzzle in-engine verification: ' + out.pass + '/' + out.total + ' levels, verdict=' + out.verdict);
