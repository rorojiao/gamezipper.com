...
**Build**: R95 (Dynamic Test Intelligence 4h cron evolution)
...
## LIBRARY METRICS (v1.12.0)
- Total test cases: **253** (was 242 in v1.9.0 visible metrics; 243 grep-anchored — see Note 1) — grep-verified via python regex anchor (R91+ Pitfall 21). R95 delta = +10 new case IDs (B-033, B-034, B-035, W-109, W-110, G-022, C-036, S-035, A-006, P-023).
- P0: 75 → 77 (+2: B-033 Chrome 149 RECORD 429 CVEs with 22 critical, B-034 Chrome Android WebGL OOB RCE CVE-2026-4439 pre-146.0.7680.153)
- P1: 94 → 102 (+8 visible: 7 R95 new + 1 surfaced by R95 sed-clean of v1.4.0 `||` pipe-typo pollution in v1.9.0 history block — see Note 1)
  - **R95 new P1** (7): B-035 WebGL UAF CVE-2026-11073 Chrome pre-149.0.7827.53, W-109 CSS gap decorations Chrome/Edge 149, W-110 Disconnect WebSockets on bfcache entry Chrome 149, G-022 AI agent pen-testing impact on game supply chain, C-036 iOS Safari `<a>`-button silent click-fail (100% iOS user impact), S-035 WCAG 2.2 2.4.11 Focus Not Obscured (April 2026 US Title II deadline), A-006 WCAG 2.2 2.5.7 Dragging + 2.5.8 Target Size 24×24
  - **Surfaced by R95 sed-clean** (1): W-038 No untrusted-input handling that mirrors Chrome 149 CVE pattern — was in v1.9.0 line 363 with `||-` prefix (v1.4.0 pitfall 10 pollution), R95 sed `s/^|- /- /g` made it match the regex anchor for the first time
- P2: 61 → 62 (+1: P-023 bfcache (Back/Forward Cache) compatibility for game state restoration)
- P3: 12 → 12 (no new P3)
- Categories: 11 (no new category; 11.11 = R95 sub-section under existing Cat 11)
- v1.12.0 delta breakdown: 2 P0 + 7 P1 + 1 P2 + 0 P3 = 10 R95 new case IDs ✓
- Note 1: v1.9.0 file's `## Version History` block had 5 lines starting with `||-` (Pitfall 10 v1.4.0 pipe-typo pollution from R89 commit). R95 ran `sed -i 's/^|- /- /g'` to clean the history block; this exposed W-038 to the regex anchor (1 case). v1.9.0 visible metrics were 242; grep-anchored reality was 243. v1.12.0 = 242 + 10 new R95 + 1 surfaced = 253 total.
- 2+7+1+0 = 10 = R95 added case IDs ✓
...
- **Note (R95 version label)**: v1.10.0 and v1.11.0 in CHANGELOG are R93/R95 sync-only entries (no master file on disk). R95 library evolution skips both to land at v1.12.0.
