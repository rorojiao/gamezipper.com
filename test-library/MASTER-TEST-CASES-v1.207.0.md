...
**Version**: 1.207.0
**Effective Date**: 2026-07-28 (v1.207.0 auto-research, 4h cycle 86)
**Build**: R324 (Dynamic Test Intelligence 4h cron evolution)

**Version Note**: v1.207.0 = R324 Dynamic Test Intelligence cron — 8 new, machine-verifiable test cases from 2026-07-28 research. This cycle covers Safari 26.6 WebAssembly JS String Builtins `compileStreaming`/`instantiateStreaming`, Safari 26.6 CSS `ic` page-zoom scaling, `position-area` body-scroll fallback, `WKHTTPCookieStore` partitioned-cookie deletion, missing-script service-worker re-registration, iPad desktop-mode CSS `zoom` font binding, `RTCPeerConnection iceTransportPolicy: "relay"` on macOS Sequoia, and Firefox 154 `RTCIceTransport.getSelectedCandidatePair()` with `selectedcandidatepairchange`.
