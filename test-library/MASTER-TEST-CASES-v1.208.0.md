...
**Version**: 1.208.0
**Effective Date**: 2026-07-29 (v1.208.0 auto-research, 4h cycle 87)
**Build**: R325 (Dynamic Test Intelligence 4h cron evolution)

**Version Note**: v1.208.0 = R325 Dynamic Test Intelligence cron — 10 new, machine-verifiable test cases from 2026-07-29 research. This cycle is Chrome 151 heavy (stable shipped Jul 28 2026, one day before this cycle) and covers: Chrome 151 `AnimationEvent.animation` / `TransitionEvent.animation` accessor reflection, CSS `position-anchor` initial-value change (`none`→`normal`), `AnimationTrigger` no-auto-rewind on completion, declarative shadow-DOM `shadowrootslotassignment` attribute + `HTMLTemplateElement.shadowRootSlotAssignment` IDL reflection, memory-safe **Rust-based XML parser** migration in non-XSLT code paths (`DOMParser`, `XMLHttpRequest.responseXML`, standalone/external SVG), `soft-navigation` + `interaction-contentful-paint` PerformanceEntry types for SPA INP, the `<usermedia>` declarative capability element gating `getUserMedia`, post-quantum WebCrypto ML-KEM / ML-DSA / ChaCha20-Poly1305 / X-Wing in Origin Trial, `DeviceOrientationEvent.requestPermission()` static API, `SpeechRecognition.unspokenPunctuation`, and the Chrome 151 / Edge 151 macOS 12 (Monterey) sunset that finally drops security updates on the oldest still-supported Apple macOS release.
