#!/usr/bin/env node
/* GENERATED in-engine verifier for samurai-sudoku — pattern follows akari/verify_engine.js.
 ...
 * Per level: initLevel(i); fill cell.value = cell.solution; checkWin() sets G.done; plus per-grid given/overlap consistency vs the 5 embedded block solutions.
 * Usage: node samurai-sudoku/verify_engine.js   (cwd = repo root)
 */
const fs = require('fs');
const vm = require('vm');
...
const DRIVER = "(function(){\nlet pass=0,fail=0,fails=[],failIdx=[];\nfor(let i=0;i<LEVELS.length;i++){const L=LEVELS[i];let why='';\n try{\n  initLevel(i);\n  const grids=[['C',L.center],['TL',L.corners.TL],['TR',L.corners.TR],['BL',L.corners.BL],['BR',L.corners.BR]];\n  let dataOK=true,overlapOK=true;\n  for(const g of grids){const gd=g[1];for(let r=0;r<9;r++)for(let c=0;c<9;c++){if(gd.puzzle[r][c]&&gd.puzzle[r][c]!==gd.solution[r][c]){dataOK=false;why='given:'+g[0]+'@'+r+','+c;}}}\n  for(let br=0;br<G.cells.length;br++)for(let bc=0;bc<G.cells.length;bc++){const cell=G.cells[br][bc];\n   if(cell&&cell.grids.size>1){for(const g of grids){const o=GRID_ORIGINS[g[0]];const r=br-o[0],c=bc-o[1];\n    if(r>=0&&r<9&&c>=0&&c<9&&g[1].solution[r][c]!==cell.solution){overlapOK=false;why='overlap@'+br+','+bc+':'+g[0];}}}}\n  for(let br=0;br<G.cells.length;br++)for(let bc=0;bc<G.cells.length;bc++){const cell=G.cells[br][bc];if(cell&&!cell.given)cell.value=cell.solution;}\n  checkWin();\n  if(G.done===true&&dataOK&&overlapOK)pass++;else{fail++;failIdx.push(i+1);fails.push('L'+(i+1)+(G.done===true?'':' noWin')+' '+why);}\n }catch(e){fail++;failIdx.push(i+1);fails.push('L'+(i+1)+' EX:'+String(e.message).slice(0,60));}\n}\nreturn {pass:pass,fail:fail,total:LEVELS.length,failIdx:failIdx,fails:fails.slice(0,15),verdict:fail===0?'PASS':'FAIL'};\n})()";
...
console.log('samurai-sudoku in-engine verification: ' + out.pass + '/' + out.total + ' levels, verdict=' + out.verdict);
