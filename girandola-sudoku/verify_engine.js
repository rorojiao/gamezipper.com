#!/usr/bin/env node
/* girandola-sudoku — permanent alias page redirecting to sudoku (no engine by design).
...
T('refresh-target', last(refresh) === 'sudoku', refresh);
T('js-target', last(js) === 'sudoku', js);
T('canonical-target', last(canon) === 'sudoku', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'sudoku', 'index.html');
...
T('target-is-real-game', tok, 'sudoku/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'sudoku' } };
