#!/usr/bin/env node
/* GENERATED in-engine verifier for knotwords — pattern follows akari/verify_engine.js.
 ...
 * IIFE-wrapped engine: surgery exports {LEVELS,startLevel,checkSolved,game}. Per level: startLevel(i+1); playerGrid = solution on grid cells; checkSolved() must be true.
 * Usage: node knotwords/verify_engine.js   (cwd = repo root)
 */
const fs = require('fs');
const vm = require('vm');
...
const SURGERY = [
 [
  ...
  "window.__VE_knot=function(){return {LEVELS:LEVELS,startLevel:startLevel,checkSolved:checkSolved,game:function(){return game;}};};\nfunction checkSolved(){"
 ]
];
...
const DRIVER = "(function(){\nconst VE=window.__VE_knot;let pass=0,fail=0,fails=[],failIdx=[];\nfor(let i=0;i<VE().LEVELS.length;i++){\n try{\n  VE().startLevel(i+1);\n  const g=VE().game();\n  for(let r=0;r<g.size;r++)for(let c=0;c<g.size;c++){if(g.grid[r][c]===1)g.playerGrid[r][c]=g.solution[r][c];}\n  const ok=VE().checkSolved()===true;\n  if(ok)pass++;else{fail++;failIdx.push(i+1);fails.push('L'+(i+1)+' noWin');}\n }catch(e){fail++;failIdx.push(i+1);fails.push('L'+(i+1)+' EX:'+String(e.message).slice(0,60));}\n}\nreturn {pass:pass,fail:fail,total:VE().LEVELS.length,failIdx:failIdx,fails:fails.slice(0,15),verdict:fail===0?'PASS':'FAIL'};\n})()";
...
console.log('knotwords in-engine verification: ' + out.pass + '/' + out.total + ' levels, verdict=' + out.verdict);
