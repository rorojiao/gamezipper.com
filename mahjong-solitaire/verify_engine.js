#!/usr/bin/env node
/* mahjong-solitaire — permanent alias page redirecting to tile-dynasty (no engine by design).
...
T('refresh-target', last(refresh) === 'tile-dynasty', refresh);
T('js-target', last(js) === 'tile-dynasty', js);
T('canonical-target', last(canon) === 'tile-dynasty', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'tile-dynasty', 'index.html');
...
T('target-is-real-game', tok, 'tile-dynasty/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'tile-dynasty' } };
