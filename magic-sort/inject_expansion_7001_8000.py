"""
Inject expansion 7001-8000 into Magic Sort index.html.
...
2. Decode all 7000 levels back to {n, b} format  
3. Read new 1000 levels from expansion txt
4. Encode all 8000 levels in base-30
...
EXPANSION_FILE = "/home/msdn/gamezipper.com/magic-sort/expansion_7001_8000.txt"
BASE = 30
...
def main():
    ...
    # 8. Update level count comment
    old_count_comment = '// ─── Level Definitions (7000 levels, compact encoding) ───'
    new_count_comment = f'// ─── Level Definitions (8000 levels, compact encoding) ───'
    ...
    # Also update the first comment line
    for old_cmt in ['// ─── Level Definitions (6000 levels) ───', 
                     '// ─── Level Definitions (7000 levels) ───']:
        if old_cmt in content:
            content = content.replace(old_cmt, '// ─── Level Definitions (8000 levels, compact encoding) ───')
    
    # 9. Add new DIFF_SECTIONS
    ...
    if diff_sections_match:
        ...
        # Find the last entry - APEX ETERNAL
        last_section_pattern = '{ start: 6901, label: "🌟 APEX ETERNAL", color: "#ff0066" }'
        if last_section_pattern in diff_content:
            new_sections = """,\n  { start: 7001, label: "🖤 VOID MASTER", color: "#0a0a2e" },\n  { start: 7151, label: "💎 PRISM LORD", color: "#ff66cc" },\n  { start: 7301, label: "⚡ STORM BREAKER", color: "#00ff66" },\n  { start: 7451, label: "🌀 CHAOS INCARNATE", color: "#ff3300" },\n  { start: 7601, label: "🔥 OBLIVION", color: "#990033" },\n  { start: 7751, label: "👑 COSMIC GOD", color: "#ff9900" },\n  { start: 7901, label: "🌟 INFINITE APEX", color: "#ff00ff" }"""
            ...
            print("Added 7 new DIFF_SECTIONS (7001-8000)", flush=True)
        else:
            print("WARNING: Could not find last DIFF_SECTION entry (APEX ETERNAL)", flush=True)
    else:
    ...
    print(f"  New DIFF_SECTIONS: +7 (7001-8000)", flush=True)
