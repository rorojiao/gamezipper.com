"""
Inject expansion 10001-11000 into Magic Sort index.html.
...
EXPANSION_FILE = "/home/msdn/gamezipper.com/magic-sort/expansion_10001_11000.txt"
BASE = 30
...
def main():
    ...
    if len(all_levels) != 11000:
        print(f"WARNING: Expected 11000 levels, got {len(all_levels)}", flush=True)
    ...
    for old_cmt in ['// ─── Level Definitions (10000 levels, compact encoding) ───']:
        if old_cmt in content:
            content = content.replace(old_cmt, '// ─── Level Definitions (11000 levels, compact encoding) ───')
            print(f"Updated comment: {old_cmt}", flush=True)
    
    # Add new DIFF_SECTIONS for 10001-11000
    ...
    if diff_sections_match:
        ...
        last_section_pattern = '{ start: 9801, label: "👑 LEGEND\'S END", color: "#ffd700" }'
        if last_section_pattern in diff_content:
            new_sections = """,
  { start: 10001, label: "💀 ELDRITCH", color: "#2a002a" },
  { start: 10201, label: "🌑 ABYSS PRIME", color: "#050510" },
  { start: 10401, label: "🌀 SINGULARITY", color: "#ff0055" },
  { start: 10601, label: "⚡ INFINITE STORM", color: "#33ff00" },
  { start: 10801, label: "👑 GOD EMPEROR", color: "#ff00ff" }"""
            ...
            print("Added 5 new DIFF_SECTIONS (10001-11000)", flush=True)
        else:
            print("WARNING: Could not find last DIFF_SECTION entry (LEGEND'S END)", flush=True)
            # Fallback
  ...
  { start: 10801, label: "👑 GOD EMPEROR", color: "#ff00ff" }"""
                ...
                content = content[:diff_sections_match.start()] + 'const DIFF_SECTIONS = [' + diff_content + '];' + content[diff_sections_match.end():]
                print("Added 5 new DIFF_SECTIONS (10001-11000) via fallback", flush=True)
    else:
    ...
    print(f"  New DIFF_SECTIONS: +5 (10001-11000)", flush=True)
    print(f"  MILESTONE: 11,000 LEVELS!")
