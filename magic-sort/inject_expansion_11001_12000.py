"""
Inject expansion 11001-12000 into Magic Sort index.html.
...
EXPANSION_FILE = "/home/msdn/gamezipper.com/magic-sort/expansion_11001_12000.txt"
BASE = 30
...
def main():
    ...
    if len(all_levels) != 12000:
        print(f"WARNING: Expected 12000 levels, got {len(all_levels)}", flush=True)
    ...
    content = content.replace(
        '// ─── Level Definitions (11000 levels, compact encoding) ───',
        '// ─── Level Definitions (12000 levels, compact encoding) ───'
    )
    print("Updated level count comment to 12000", flush=True)
    
    # Add new DIFF_SECTIONS for 11001-12000
    ...
    if diff_sections_match:
        ...
        last_section_pattern = '{ start: 10801, label: "GOD EMPEROR", color: "#ff00ff" }'
        if last_section_pattern in diff_content:
            new_sections = """,
  { start: 11001, label: "OMEGA PRIME", color: "#ff3300" },
  ...
  { start: 11801, label: "ZENITH ASCENDED", color: "#ccff00" }"""
            ...
            print("Added 5 new DIFF_SECTIONS (11001-12000)", flush=True)
        else:
            print("WARNING: Could not find last DIFF_SECTION entry (GOD EMPEROR)", flush=True)
    else:
        print("WARNING: Could not find DIFF_SECTIONS", flush=True)
    
    # Update SEO meta tags
    content = content.replace('11,000', '12,000')
    content = content.replace('11000', '12000')
    print("Updated SEO meta tags to 12,000", flush=True)
    
    # Update achievements count in description
    content = content.replace('18+ achievements', '19+ achievements')
    
    # Add new milestone emojis
    if 'if (lv === 11500) return' not in content:
        content = content.replace(
            'if (lv === 11000) return "\\u{1F31F}";',
            'if (lv === 11000) return "\\u{1F31F}";\n  if (lv === 11500) return "\\u{1F48E}";\n  if (lv === 12000) return "\\u{1F525}";'
        )
        print("Added 11500/12000 milestone emojis", flush=True)
    
    # Add new achievement for 12K
    if 'twelve_k' not in content:
        content = content.replace(
            '{ id: "eleven_k", name: "APEX ASCENDANT"',
            '{ id: "twelve_k", name: "TRANSCENDENT GOD", desc: "Complete all 12,000 levels", icon: "\\u{1F525}", check: (s) => s.completed >= 12000 },\n  { id: "eleven_k", name: "APEX ASCENDANT"'
        )
        print("Added TRANSCENDENT GOD achievement (12K)", flush=True)
    ...
    print(f"  New DIFF_SECTIONS: +5 (11001-12000)", flush=True)
    print(f"  MILESTONE: 12,000 LEVELS!")
