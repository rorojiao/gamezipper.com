"""
Inject expansion 8001-9000 into Magic Sort index.html.
...
2. Decode all 8000 levels back to {n, b} format  
3. Read new 1000 levels from expansion txt
4. Encode all 9000 levels in base-30
...
EXPANSION_FILE = "/home/msdn/gamezipper.com/magic-sort/expansion_8001_9000.txt"
BASE = 30
...
def main():
    ...
    for old_cmt in ['// ─── Level Definitions (6000 levels) ───', 
                     ...
                     '// ─── Level Definitions (8000 levels) ───',
                     '// ─── Level Definitions (8000 levels, compact encoding) ───']:
        if old_cmt in content:
            content = content.replace(old_cmt, '// ─── Level Definitions (9000 levels, compact encoding) ───')
    print("Updated level count comment", flush=True)
    
    # 9. Add new DIFF_SECTIONS for 8001-9000
    ...
    if diff_sections_match:
        ...
        # Find the last entry - INFINITE APEX
        last_section_pattern = '{ start: 7901, label: "🌟 INFINITE APEX", color: "#ff00ff" }'
  ...
  { start: 8801, label: "👑 ABSOLUTE ZERO", color: "#00cccc" }"""
            ...
            print("Added 7 new DIFF_SECTIONS (8001-9000)", flush=True)
        else:
            print("WARNING: Could not find last DIFF_SECTION entry (INFINITE APEX)", flush=True)
            print("Attempting to find and append to last section...")
            # Fallback: find last section and append after it
  ...
  { start: 8801, label: "👑 ABSOLUTE ZERO", color: "#00cccc" }"""
                ...
                content = content[:diff_sections_match.start()] + 'const DIFF_SECTIONS = [' + diff_content + '];' + content[diff_sections_match.end():]
                print("Added 7 new DIFF_SECTIONS (8001-9000) via fallback", flush=True)
    else:
    ...
    print(f"  New DIFF_SECTIONS: +7 (8001-9000)", flush=True)
