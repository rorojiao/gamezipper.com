"""
Inject expansion 14001-15000 into Magic Sort index.html.
...
EXPANSION_FILE = "/home/msdn/gamezipper.com/magic-sort/expansion_14001_15000.txt"
BASE = 30
...
def main():
    ...
    expected = 15000
    if len(all_levels) != expected:
        print(f"WARNING: Expected {expected} levels, got {len(all_levels)}", flush=True)
    ...
    content = content.replace(
        '// ─── Level Definitions (14000 levels, compact encoding) ───',
        '// ─── Level Definitions (15000 levels, compact encoding) ───'
    )
    # Also try without trailing dashes in case format varies
    if 'Level Definitions (14000 levels' in content:
        content = content.replace('14000 levels', '15000 levels')
    print("Updated level count comment to 15000", flush=True)
    
    # Add new DIFF_SECTIONS for 14001-15000
    ...
    if diff_sections_match:
        ...
        last_section_pattern = '{ start: 13801, label: "🔮 TRANSCENDENT VOID", color: "#9900ff" }'
        if last_section_pattern in diff_content:
            new_sections = """,
  { start: 14001, label: "🌈 CELESTIAL PRISM", color: "#ff69b4" },
  { start: 14201, label: "💀 ABYSS WALKER", color: "#4a0000" },
  { start: 14401, label: "⚡ QUANTUM STORM", color: "#00ffff" },
  { start: 14601, label: "🔥 INFERNO CORE", color: "#ff4500" },
  { start: 14801, label: "🌌 OMNIVERSE EDGE", color: "#4169e1" }"""
            ...
            print("Added 5 new DIFF_SECTIONS (14001-15000)", flush=True)
        else:
            print("WARNING: Could not find last DIFF_SECTION entry (TRANSCENDENT VOID)", flush=True)
    else:
        print("WARNING: Could not find DIFF_SECTIONS", flush=True)
    
    # Update SEO meta tags
    content = content.replace('14,000', '15,000')
    content = re.sub(r'14000 levels', '15000 levels', content)
    print("Updated SEO meta tags to 15,000", flush=True)
    
    # Add new milestone emojis (after 14000 👑)
    if 'if (lv === 14500) return' not in content:
        content = content.replace(
            'if (lv === 14000) return "👑";',
            'if (lv === 14000) return "👑";\n  if (lv === 14500) return "🔮";\n  if (lv === 15000) return "🏆";'
        )
        print("Added 14500/15000 milestone emojis", flush=True)
    
    # Add new achievement for 15K
    if 'fifteen_k' not in content:
        content = content.replace(
            '{ id: "fourteen_k", name: "COSMIC ARCHITECT"',
            '{ id: "fifteen_k", name: "OMNIVERSE MASTER", desc: "Complete all 15,000 levels", icon: "\\u{1F3C6}", check: (s) => s.completed >= 15000 },\n  { id: "fourteen_k", name: "COSMIC ARCHITECT"'
        )
        print("Added OMNIVERSE MASTER achievement (15K)", flush=True)
    ...
    print(f"  New DIFF_SECTIONS: +5 (14001-15000)")
    print(f"  MILESTONE: 15,000 LEVELS!")
