"""
Inject expansion 13001-14000 into Magic Sort index.html.
...
EXPANSION_FILE = "/home/msdn/gamezipper.com/magic-sort/expansion_13001_14000.txt"
BASE = 30
...
def main():
    ...
    if len(all_levels) != 14000:
        print(f"WARNING: Expected 14000 levels, got {len(all_levels)}", flush=True)
    ...
    print("Updated level count comment to 14000", flush=True)
    
    # Add new DIFF_SECTIONS for 13001-14000
    ...
    if diff_sections_match:
        ...
        last_section_pattern = '{ start: 12801, label: "👑 APEX TRANSCEND", color: "#ffcc00" }'
        if last_section_pattern in diff_content:
            new_sections = """,
  { start: 13001, label: "💀 COSMIC OBLIVION", color: "#1a001a" },
  { start: 13201, label: "🌀 SINGULARITY PRIME", color: "#ff0055" },
  { start: 13401, label: "⚡ INFINITE SURGE", color: "#33ff00" },
  { start: 13601, label: "👑 ETERNAL SOVEREIGN", color: "#ffd700" },
  { start: 13801, label: "🔮 TRANSCENDENT VOID", color: "#9900ff" }"""
            ...
            print("Added 5 new DIFF_SECTIONS (13001-14000)", flush=True)
        else:
            print("WARNING: Could not find last DIFF_SECTION entry (APEX TRANSCEND)", flush=True)
    else:
        print("WARNING: Could not find DIFF_SECTIONS", flush=True)
    
    # Update SEO meta tags
    content = content.replace('13,000', '14,000')
    content = re.sub(r'13000 levels', '14000 levels', content)
    print("Updated SEO meta tags to 14,000", flush=True)
    
    # Update achievements count
    content = content.replace('20+ achievements', '21+ achievements')
    
    # Add new milestone emojis
    if 'if (lv === 13500) return' not in content:
        content = content.replace(
            'if (lv === 13000) return "\\u{1F48E}";',
            'if (lv === 13000) return "\\u{1F48E}";\n  if (lv === 13500) return "\\u{1F31F}";\n  if (lv === 14000) return "\\u{1F451}";'
        )
        print("Added 13500/14000 milestone emojis", flush=True)
    
    # Add new achievement for 14K
    if 'fourteen_k' not in content:
        content = content.replace(
            '{ id: "thirteen_k", name: "INFINITE SOVEREIGN"',
            '{ id: "fourteen_k", name: "COSMIC ARCHITECT", desc: "Complete all 14,000 levels", icon: "\\u{1F451}", check: (s) => s.completed >= 14000 },\n  { id: "thirteen_k", name: "INFINITE SOVEREIGN"'
        )
        print("Added COSMIC ARCHITECT achievement (14K)", flush=True)
    ...
    print(f"  New DIFF_SECTIONS: +5 (13001-14000)", flush=True)
    print(f"  MILESTONE: 14,000 LEVELS!")
