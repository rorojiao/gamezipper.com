#!/usr/bin/env node
/* numberlink — permanent alias page redirecting to flow-connect (no engine by design).
...
T('refresh-target', last(refresh) === 'flow-connect', refresh);
T('js-target', last(js) === 'flow-connect', js);
T('canonical-target', last(canon) === 'flow-connect', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'flow-connect', 'index.html');
...
T('target-is-real-game', tok, 'flow-connect/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'flow-connect' } };
