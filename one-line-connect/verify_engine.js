#!/usr/bin/env node
/* one-line-connect — permanent alias page redirecting to one-line-puzzle (no engine by design).
...
T('refresh-target', last(refresh) === 'one-line-puzzle', refresh);
T('js-target', last(js) === 'one-line-puzzle', js);
T('canonical-target', last(canon) === 'one-line-puzzle', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'one-line-puzzle', 'index.html');
...
T('target-is-real-game', tok, 'one-line-puzzle/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'one-line-puzzle' } };
