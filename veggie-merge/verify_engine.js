#!/usr/bin/env node
/* veggie-merge — permanent alias page redirecting to watermelon-merge (no engine by design).
...
T('refresh-target', last(refresh) === 'watermelon-merge', refresh);
T('js-target', last(js) === 'watermelon-merge', js);
T('canonical-target', last(canon) === 'watermelon-merge', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'watermelon-merge', 'index.html');
...
T('target-is-real-game', tok, 'watermelon-merge/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'watermelon-merge' } };
