#!/usr/bin/env node
/* butterfly-sudoku — permanent alias page redirecting to samurai-sudoku (no engine by design).
...
T('refresh-target', last(refresh) === 'samurai-sudoku', refresh);
T('js-target', last(js) === 'samurai-sudoku', js);
T('canonical-target', last(canon) === 'samurai-sudoku', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'samurai-sudoku', 'index.html');
...
T('target-is-real-game', tok, 'samurai-sudoku/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'samurai-sudoku' } };
