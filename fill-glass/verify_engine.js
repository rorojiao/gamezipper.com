#!/usr/bin/env node
/* fill-glass — permanent alias page redirecting to happy-glass (no engine by design).
...
T('refresh-target', last(refresh) === 'happy-glass', refresh);
T('js-target', last(js) === 'happy-glass', js);
T('canonical-target', last(canon) === 'happy-glass', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'happy-glass', 'index.html');
...
T('target-is-real-game', tok, 'happy-glass/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'happy-glass' } };
