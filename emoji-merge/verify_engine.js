#!/usr/bin/env node
/* emoji-merge — permanent alias page redirecting to threes (no engine by design).
...
T('refresh-target', last(refresh) === 'threes', refresh);
T('js-target', last(js) === 'threes', js);
T('canonical-target', last(canon) === 'threes', canon);
T('no-engine-by-design', !/<canvas|requestAnimationFrame/.test(html), 'alias page should not embed an engine');
const tp = path.join(__dirname, '..', 'threes', 'index.html');
...
T('target-is-real-game', tok, 'threes/index.html');
const out = { pass, fail, total: pass + fail, verdict: fail === 0 ? 'PASS' : 'FAIL', fails, extra: { aliasOf: 'threes' } };
